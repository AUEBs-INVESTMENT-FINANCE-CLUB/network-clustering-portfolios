...
All portfolios are evaluated on the same set of stocks. The investable universe is the set of stocks with a valid price at the start of the test window and sufficient data in the training window (e.g.\ 90\% non-missing); no information from the test window beyond the existence of a tradable price at $t_0$ is used to define the universe. The universe is unchanged across training and test periods, so differences in outcomes stem from the allocation rules rather than from differing constituents.

All computations use daily closing prices from LSEG Reuters Workspace. Annualization follows a 252 trading-day convention. The evaluation window includes stressed periods such as Brexit-related uncertainty and the COVID-19 shock, which provides a natural stress test for dependence-based diversification under a strict buy-and-hold design.
...
We focus on first-moment and volatility-based metrics plus diversification ratio; downside-risk measures are left for future work. Table~\ref{tab:metrics_oos} reports annualised mean return, volatility, Sharpe ratio, and diversification ratio (out-of-sample asset returns and fixed weights).
