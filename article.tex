...
We use a split-sample design. Portfolio weights are estimated on an initial training window and then kept fixed over a later test window. We use 2015--2017 as the training window and 2018--2025 as the test window. We do not rebalance in the test phase, so the comparison reflects how each diversification rule behaves when exposures are held constant.

All portfolios are evaluated on the same set of stocks. The investable universe is the set of stocks with a valid price at the start of the test window and sufficient data in the training window.Also all computations use daily closing prices from LSEG Reuters Workspace. Annualization follows a 252 trading-day convention. The evaluation window includes stressed periods such as uncertainty related to Brexit and COVID-19 providing some stress tests for diversification under a buy-and-hold regime.
...
The threshold $\tau$ is set at the 80th percentile of the off-diagonal correlations, so the network emphasises the strongest relationships while controlling sparsity \cite{Tumminello2005}.
...
Eigenvector centrality measures how connected a node is to other well-connected nodes. Highly central stocks tend to lie in the core of the dependence structure. To diversify, we use inverse centrality: we assign greater weight to less central (less systemic) stocks and reduce exposure to the core while remaining fully invested \cite{Peralta2016}.
...
\end{equation}
We build a dendrogram with average linkage and cut it into $K=15$ clusters. Denote by $c(i)$ the cluster of stock $i$ and by $|C_{c(i)}|$ the size of that cluster. We allocate equal capital to each cluster and equal capital within each cluster:
\begin{equation}
...
\end{equation}
We cluster the points $(\hat{\mu}_i,\hat{\sigma}_i)$ into $K=4$ clusters and score each cluster by its average return-to-volatility ratio:
\begin{equation}
\text{Score}(c) = \frac{\overline{\hat{\mu}}_c}{\overline{\hat{\sigma}}_c}.
\end{equation}
We select the top-scoring clusters subject to a minimum cluster size. Capital is allocated across selected clusters in proportion to their non-negative scores, with equal weights within each selected cluster. K-means is run with a fixed seed for reproducibility.
...
Let $R^{(p)}_t$ and $R^{(b)}_t$ be the daily simple returns of the portfolio and the benchmark. Dependence estimates $(\bm{\rho},\bm{\Sigma})$ are computed from training-period daily log returns, while $R^{(p)}_t$ is the daily simple return implied by the buy-and-hold wealth path $V_t$. Annualised mean return and volatility are
\begin{equation}
...
\end{equation}
For strategies with constituent weights, we report the diversification ratio $\mathrm{DR} = (\bm{w}^\top \bm{\sigma}) / \sqrt{\bm{w}^\top \bm{\Sigma} \bm{w}}$, where $\bm{\sigma}$ is the vector of asset volatilities and $\bm{\Sigma}$ the covariance matrix of asset returns.
...
We focus on first-moment and volatility-based metrics plus diversification ratio; downside-risk measures are left for future work. Table~\ref{tab:metrics_oos} reports annualized mean return, volatility, Sharpe ratio, and diversification ratio (out-of-sample asset returns and fixed weights).
...
Portfolio & \multicolumn{1}{c}{MeanRet (\%)} & \multicolumn{1}{c}{Vol (\%)} & \multicolumn{1}{c}{Sharpe} & \multicolumn{1}{c}{DR} \\
\hline
FTSE 100        & 4.05  & 15.424 & 0.263 & \multicolumn{1}{c}{--} \\
Degeneracy      & 9.44  & 16.60 & 0.56 & 1.90 \\
Inv.\ eigen.\   & 7.54  & 15.79 & 0.47 & 1.85 \\
Cluster equal   & 12.34 & 18.15 & 0.68 & 1.86 \\
K-means         & 8.72  & 18.07 & 0.48 & 1.65 \\
HERC            & 7.15  & 15.13 & 0.47 & 1.83 \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
Some extensions could include multiplex networks that encode tail or rank dependence in addition to linear correlation \cite{JaegerMarinelli2022}, and combining inverse centrality with volatility weighting (e.g. inverse eigenvector centrality with inverse volatility). The effective number of uncorrelated bets could be reported in future work.

In practice, implementation would need to allow for transaction costs, bid--ask spreads, taxes, corporate actions, and index membership changes, and would typically involve periodic rebalancing and constraints on turnover and position size.
